...
══════════════════════════════════════════ */
(function() {
  if (window.emailjs) { emailjs.init({ publicKey: EJ_KEY }); }
})();
...
// Animate the progress bar with a natural easing curve
var plProgress = 0;
var plTarget   = 0;
var plRaf      = null;

function setPlTarget(t) {
  plTarget = Math.min(t, 100);
  if (!plRaf) plRaf = requestAnimationFrame(stepPl);
}

function stepPl() {
  plProgress += (plTarget - plProgress) * 0.07;
  if (Math.abs(plTarget - plProgress) < 0.25) plProgress = plTarget;
  var pct = Math.round(plProgress);
  if (plFill)   plFill.style.width = pct + '%';
  if (plPct)    plPct.textContent  = pct + '%';
  // Update status message
  if (plStatus) {
    var idx = pct < 20 ? 0 : pct < 45 ? 1 : pct < 72 ? 2 : pct < 95 ? 3 : 4;
    plStatus.textContent = statusMessages[idx];
  }
  if (plProgress < plTarget) {
    plRaf = requestAnimationFrame(stepPl);
  } else {
    plRaf = null;
  }
}

// Drive progress in stages matching real page load
setTimeout(function() { setPlTarget(30); }, 80);
setTimeout(function() { setPlTarget(58); }, 420);
setTimeout(function() { setPlTarget(78); }, 900);
setTimeout(function() { setPlTarget(92); }, 1600);
...
function hidePremiumLoader() {
  ...
  setTimeout(function() {
    ...
    // Fire the page reveal the moment the loader starts fading
...
window.addEventListener('scroll', function() {
  if (!parallaxRaf) {
    parallaxRaf = requestAnimationFrame(function() {
      ...
      // Fade hero on scroll
      if (heroSection && document.body.classList.contains('site-ready')) {
